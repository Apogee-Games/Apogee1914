...
void FMapManager::CreateOutline()
{
	...
	for (int i = 0; i < GetMapSize(); ++i)
	{
		...
		if (x > 0 && ProvincesColor[i] != ProvincesColor[i - 1] &&
			GameState && GameState->GetProvince(ProvincesColor[i]).StateId != GameState->GetProvince(
				ProvincesColor[i - 1]).StateId)
			OutlineColors[i] = OutlineDefaultColor;
		else if (x + 1 < SizeVector.X && ProvincesColor[i] != ProvincesColor[i + 1] &&
			GameState && GameState->GetProvince(ProvincesColor[i]).StateId != GameState->GetProvince(
				ProvincesColor[i + 1]).StateId)
			OutlineColors[i] = OutlineDefaultColor;
		else if (y > 0 && ProvincesColor[i] != ProvincesColor[i - static_cast<int>(SizeVector.X)] &&
			GameState && GameState->GetProvince(ProvincesColor[i]).StateId != GameState->GetProvince(
				ProvincesColor[i - static_cast<int>(SizeVector.X)]).StateId)
			OutlineColors[i] = OutlineDefaultColor;
		else if (y + 1 < SizeVector.Y && ProvincesColor[i] != ProvincesColor[i + static_cast<int>(SizeVector.X)] &&
			GameState && GameState->GetProvince(ProvincesColor[i]).StateId != GameState->GetProvince(
				ProvincesColor[i + static_cast<int>(SizeVector.X)]).StateId)
			OutlineColors[i] = OutlineDefaultColor;
